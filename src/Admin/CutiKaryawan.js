...
function CutiKaryawan() {
  ...
  useEffect(() => {
    ...
    // Get existing cuti data
    const existingCuti = Array.isArray(cutiData) ? cutiData : [];
    const existingKaryawanNames = existingCuti
      .map((c) => c?.nama)
      .filter((name) => name && name.trim() !== "");
    
    // Find karyawan that don't have libur data yet
    const karyawanWithoutLibur = uniqueKaryawan.filter(
      (nama) => !existingKaryawanNames.includes(nama)
    );
    
    if (karyawanWithoutLibur.length === 0) return;
    
    // Generate libur data for each karyawan
    const newLiburData = [];
    const alasanOptions = [
      "Libur Tahunan",
      "Keperluan Pribadi",
    ...
    karyawanWithoutLibur.forEach((nama, index) => {
      // Create 2-3 libur entries per karyawan with different dates
      const numEntries = 2 + (index % 2); // 2 or 3 entries
      
      for (let i = 0; i < numEntries; i++) {
        const daysAgo = 30 + (index * 7) + (i * 10); // Different dates for each entry
        const liburDate = new Date(today);
        liburDate.setDate(liburDate.getDate() - daysAgo);
        const tanggal = liburDate.toISOString().split('T')[0];
        
        const alasan = alasanOptions[(index + i) % alasanOptions.length];
        const lama = 1 + (index % 3); // 1, 2, or 3 days
        const status = statusOptions[(index + i) % statusOptions.length];
        
        newLiburData.push({
          id: `libur-${nama.replace(/\s+/g, '-').toLowerCase()}-${index}-${i}-${Date.now()}`,
          nama: nama,
          tanggal: tanggal,
          lama: lama,
          alasan: alasan,
    ...
    // Add new libur data to existing data
